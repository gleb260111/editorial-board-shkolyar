import{s as g,c as l,o as h,e as r,g as p,r as d,d as c,h as m,u as w,a as f}from"./firebase-init-BcDGRJEx.js";window.innerWidth>900&&window.location.replace("dashboard.html");const v=document.getElementById("mobile-admin-panel"),y=document.getElementById("menu-logout-btn"),b=document.getElementById("close-menu-btn");b.addEventListener("click",()=>{window.history.back()});y.addEventListener("click",()=>{g(l).then(()=>window.location.href="index.html")});h(l,async t=>{if(t){r();try{const o=await p(d(c,"roles/"+t.uid));o.exists()&&o.val()==="admin"&&E(t.uid)}catch(o){console.error(o)}finally{m()}}else window.location.href="index.html"});async function E(t){v.classList.remove("hidden"),((e,a)=>{const i=document.getElementById(e);i.checked=localStorage.getItem(a)!=="false",i.addEventListener("change",u=>localStorage.setItem(a,u.target.checked))})("mob-singularity","pref_show_singularity");const n=document.getElementById("mob-raw-cosmetics");n&&(n.checked=localStorage.getItem("pref_raw_cosmetics")==="true",n.addEventListener("change",e=>localStorage.setItem("pref_raw_cosmetics",e.target.checked)));const s=(await p(d(c,`users/${t}/equipped`))).val()||{};if(s.title==="accountant"){const e=s.title_override||"",a=document.getElementById("mob-accountant-wrapper");a.innerHTML=`
