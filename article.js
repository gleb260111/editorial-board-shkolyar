import { 
    ...
    ref, get, update, increment, query, orderByChild, equalTo, onAuthStateChanged, SHOP_ITEMS_BASE
...
const loadArticle = async () => {
    if (!articleId) {
        document.getElementById('article-container').innerHTML = '<h2 style="text-align:center;">Статья не найдена</h2>';
        return;
    }
    try {
        ...
        const article = { id: snapshot.key, ...snapshot.val() };
        renderArticle(article);
    } catch (e) {
        console.error(e);
        document.getElementById('article-container').innerHTML = '<h2 style="text-align:center;">Ошибка загрузки</h2>';
    }
};

const renderArticle = (article) => {
    const container = document.getElementById('article-container');
    const date = new Date(article.publishAt || article.createdAt).toLocaleDateString('ru-RU');
    ...
    if (photos.length > 0 || article.videoUrl) {
        topLinksHtml = `<div class="article-images-list">`;
    ...
    card.innerHTML = `
        ...
        ${topLinksHtml}
        <h3 style="margin-top: 10px; font-size: 1.8rem;">${escapeHTML(article.title)}</h3>
        <p class="article-text" style="max-height:none; mask-image:none; font-size: 1.1rem;">${parseMarkdown(article.text)}</p>
        ${inlineImagesHtml}
        <div class="article-meta" style="margin-top: 25px; justify-content: space-between;">
            <span>📅 Опубликовано: ${date}</span>
            <div class="feed-actions" style="margin:0; border:none; padding:0; gap: 10px;">
                <button class="btn-share" id="btn-share" title="Поделиться">
                    🔗 Поделиться
                </button>
    ...
    loadAuthorData(article.authorId, card);

    const likeBtn = card.querySelector('.btn-like');
    likeBtn.addEventListener('click', (e) => handleLike(article.id, article.authorId, e));
...
const loadAuthorData = async (authorId, cardElement) => {
    try {
        const [userSnap, overrideSnap] = await Promise.all([
        ...
        if (titleVal) htmlContent += `<span class="rank-tag title-tag ${specialClass}">${escapeHTML(titleVal)}</span>`;
...
window.openDossier = async (uid) => {
    ...
    try {
        ...
        if (equipped.aura) visualClasses += ' ' + equipped.aura;
        if (equipped.aura_reverse) visualClasses += ' ' + equipped.aura_reverse;
        ...
        const avatarHTML = `
            <div class="${visualClasses}" style="width: 100px; height: 100px; margin: 0 auto 15px auto; position: relative; overflow: visible; border-radius: 50%; border: ${equipped.border ? 'none' : '3px solid var(--border-color)'};">
                <div class="aura-container">${innerImg}</div>
            </div>
        `;
        let titleHTML = '';
        ...
        if (displayTitle) {
            if (displayTitle.toString().toLowerCase() === 'accountant') {
                if (equipped.title_override) displayTitle = equipped.title_override;
                else {
                    const bal = stats.balance || 0;
            ...
            } else if (displayTitle === 'Повелитель Света') specialClass = 'special-light';
            else if (displayTitle === 'Владыка Тьмы') specialClass = 'special-dark';
        ...
        content.innerHTML = `
            <div class="dossier-header">
                ...
                <div style="margin-top: 10px;">${titleHTML}</div>
            </div>
            <div class="dossier-stats">
                <div class="dossier-stat-item"><span class="dossier-stat-val">${stats.xp || 0}</span><span class="dossier-stat-label">XP</span></div>
                <div class="dossier-stat-item"><span class="dossier-stat-val">❤️ ${stats.totalLikes || 0}</span><span class="dossier-stat-label">Репутация</span></div>
                <div class="dossier-stat-item"><span class="dossier-stat-val">📝 ${articlesCount}</span><span class="dossier-stat-label">Статей</span></div>
            </div>
        `;
    } catch (e) {
        console.error(e);
